;(function($){
	...
	methods = {
		init: function(options){
			...
			return this.each(function(){
				...
				}else if(/\.(?:com|net|info|edu|co|in|uk|us|mobi|org|ly|biz|fr|de|es|it)/.test(url)){
						getData(url);
					}
				}
				...
				function draw(set){
					...
					$.each(set, function(key, val){			
						if(val){
							if(key == 'image'){
                                var arr_url = set.url.split('/');
                                var correct_url = 'http://'+arr_url[2]+'/';
								img = $('<img/>', {src: correct_url + val});
					...
					$('.postArea div.liveContainer').html(anchor.append(imgWrapper, textWrapper));
	...
	$.fn.urlive = function(method){
		...
		if(methods[method] && (this.ajaxCalled == false || typeof this.ajaxCalled == 'undefined')){
		
			return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
		}else if(typeof method === 'object' || !method && (this.ajaxCalled == false || typeof this.ajaxCalled == 'undefined')){
