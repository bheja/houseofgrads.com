        ...
        this.applyText = function(text) {
            ...
            if (this.options.words.length > 0) {
             ...
             // for(i=0;i<this.options.words.length;i++){
			   // var x = this.options.words[i];
                // text = text.replace(
                  // new RegExp(x+'(?!.*'+x+')', this.options.regParam), 
                  // "<span class=\"highlight2\">"+x+"</span>"
                // );
				if(this.options.comment == false){
				carArr = $.trim($('#caretPosition').val()).split(' ');
				while (i<carArr.length-1){
				text = text.substring(carArr[i+2],carArr[i+3]);
				}
				//}
				}
