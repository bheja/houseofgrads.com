...
qq.FileUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        template: '<div class="qq-uploader clearfix">' +
                '<div class="btn btn-default qq-upload-button floatL" title="Upload an Image"><span class="glyphicon glyphicon-paperclip largeFont"></span></div>' +
             '</div>',

        // template for one item in file list
        fileTemplate: '<div class="marginT progress progress-striped active"><div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100">' +
...
qq.extend(qq.FileUploader.prototype, {
    ...
    _onProgress: function(id, fileName, loaded, total){
        ...
        var text; 
			...
			var txt = new String(text);
			 ...
			 $(size).parents('.progress-bar-success').attr('aria-valuenow',txt);
			$(size).parents('.progress-bar-success').css('width',txt+'%');                 
            var text2 = this._formatSize(total);   
		if (txt != '100'){
	   $(size).text(txt+'%');}else{$(size).text(text2);}
    },
    _onComplete: function(id, fileName, result){
        ...
        if (result.success){
            ...
            var url_check =document.URL; var flag_blog = "false";
            if (url_check.indexOf('blog')!=-1){flag_blog = "true";}
			$(file).parents('.progress').addClass('alert alert-success').removeClass('progress-striped active progress');
				$(file).parents('.progress-bar-success').removeClass('progress-bar-success progress-bar');
				$(file).before('<a href="#" class="floatR alert-link removeUploaded" onclick="remove_uploaded(this,'+this._options['multiple']+','+flag_blog+');isblog('+flag_blog+');" title="remove"><span class="glyphicon glyphicon-remove"></span></a>');
        } else {
            qq.addClass(item, this._classes.fail);
			$(file).parents('.progress').addClass('alert alert-warning').removeClass('progress-striped active progress');
                 $(file).parents('.progress-bar-success').removeClass('progress-bar-success progress-bar');
				 $(file).before('<a href="#" class="floatR alert-link removeUploaded" onclick="remove_uploaded(this,'+this._options['multiple']+')" title="remove"><span class="glyphicon glyphicon-remove"></span></a>');
		}
    },
    _addToList: function(id, fileName){
		...
		var file_name = this._formatFileName(fileName);
