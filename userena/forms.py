    ...
    def clean_username(self):
        ...
        try:
            ...
            if userena_settings.USERENA_ACTIVATION_REQUIRED and UserenaSignup.objects.filter(user__username__iexact=self.cleaned_data['username']).exclude(activation_key=userena_settings.USERENA_ACTIVATED):
                raise forms.ValidationError(_('This nick is already registered. If you forgot password, click <a class="boldText colorRed" href="/accounts/password/reset/">here</a> to reset.'))
            raise forms.ValidationError(_('This nick is already registered. If you forgot password, click <a class="boldText colorRed" href="/accounts/password/reset/">here</a> to reset.'))
    ...
    def clean_email(self):
        ...
        if get_user_model().objects.filter(email__iexact=self.cleaned_data['email']):
            if userena_settings.USERENA_ACTIVATION_REQUIRED and UserenaSignup.objects.filter(user__email__iexact=self.cleaned_data['email']).exclude(activation_key=userena_settings.USERENA_ACTIVATED):
                raise forms.ValidationError(_('This email is already registered. If you forgot password, click <a class="boldText colorRed" href="/accounts/password/reset/">here</a> to reset.'))
            raise forms.ValidationError(_('This email is already in use. Click <a class="boldText colorRed" href="/accounts/password/reset/">here</a> if you have forgotten the password.'))
